...
def memory_storage():
    ...
    storage = Storage.__new__(Storage)  # bypass __init__
    storage.db_path = ":memory:"
    storage._connect = lambda: conn
    storage._init_db()  # create tables
    yield storage
    conn.close()

def test_insert_and_get_last_check(memory_storage):
...
def test_summary_calculates_uptime_percentage(memory_storage):
    storage = memory_storage
    storage.insert_check(make_result("UP", "t1"))
    storage.insert_check(make_result("DOWN", "t2"))
